...
bool ScanEngulfsInTimeframeRange(const string symbol,
    ...
    if (!CopyHtfRates(symbol, analysisTimeframe, htf_count, tf_open, tf_high, tf_low, tf_close, tf_time))
        return false;
    ...
    for (int i = iStart; i <= scanEnd; i++) {
        EngulfInfo info = DetectBearishEngulfAt(i, tf_open, tf_high, tf_low, tf_close, tf_time);
        if (info.found)
